...
class MemoryNetworkLayer {
    ...
    init() {
        NotificationCenter.default.addObserver(forName: Memory.InstanceChangedNotification, object: nil, queue: nil) { n in
            ...
            for id in diffs {
                self.sendInstanceToNetwork(id, createIfNeeded: false) { n,e in
                    ...
                    if let error = e {
                        if let netErr = error as? Network.NetworkError, netErr == Network.NetworkError.mergeSupplanted {
                            ...
                            self.sendNetworkToInstance(n, createIfNeeded: false, continuingAfterMergeConflict: true) { mid,e in
                                if let error = e {
        ...
        NotificationCenter.default.addObserver(forName: Network.FileChangedNotification, object: nil, queue: nil) { n in
            ...
            for id in ids {
                DataStack.sharedInstance.network.getFile(id) { pair in
                    if pair == nil {
                        print("deletion sync not yet implemented")
                    }
                    else {
                        self.sendNetworkToInstance(id, createIfNeeded: false, continuingAfterMergeConflict: false) { mid,e in
                            if let error = e {
    ...
    public func sendNetworkToInstance(_ id: Network.FileID, createIfNeeded: Bool, continuingAfterMergeConflict: Bool, _ block: @escaping (Memory.InstanceID, Error?)->()) {
        DataStack.sharedInstance.network.getFile(id) { p in
    ...
    public func sendInstanceToNetwork(_ id: Memory.InstanceID, createIfNeeded: Bool, _ block: @escaping (Network.FileID, Error?)->()) {
        ...
        if let networkId = mappingMN[id] {
            DataStack.sharedInstance.network.merge(networkId, data) { e in
                if let error = e {
            ...
            DataStack.sharedInstance.network.create(file: data, named: Date().description) { m,e in
                if let error = e {
                    assert(false)
    ...
    public func delete(_ nid: Network.FileID, _ block: @escaping (Error?)->()) {
        ...
        DataStack.sharedInstance.network.delete(nid) { e in
            block(e)
        }
    }
...
//    }
    ...
    public func convertNetworkToMemory(_ n: Data) -> CRDTTextEditing {
        let maybeUncompressedData = n.withUnsafeBytes { (sourceBuffer: UnsafePointer<UInt8>) -> Data? in
